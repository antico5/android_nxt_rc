...
public class LeJOSDroid extends Activity {
	...
	public static NXTConnector connect(final CONN_TYPE connection_type) {
		...
		switch (connection_type) {
			case LEGO_LCP:
				boolean result=conn.connectTo("btspp://NXT", NXTComm.LCP);
				if(!result)return null;
				break;
			case LEJOS_PACKET:
				boolean result2=conn.connectTo("btspp://");
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		LeJOSDroid.displayToastOnUIThread("Iniciado");
	}
	...
	private void setupAndroidRC(final LeJOSDroid leJOSDroid) {
		Button button;
		...
		button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				Intent AndroidRC = new Intent(leJOSDroid, lejos.android.AndroidRC.class);
				startActivity(AndroidRC);
			}
		});
	}
